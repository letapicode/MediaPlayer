...
static int countFor(sqlite3 *db, const std::string &path) {
  sqlite3_stmt *stmt = nullptr;
  sqlite3_prepare_v2(db, "SELECT COUNT(*) FROM MediaItem WHERE path=?1;", -1, &stmt, nullptr);
...
int main() {
  const char *dbPath = "purge.db";
  const std::string video = "purge_test.mp4";
  createTestMp4(video);
  ...
  sqlite3_open(dbPath, &conn);
  int remaining = countFor(conn, video);
  sqlite3_close(conn);
  std::remove(dbPath);
  assert(remaining == 0 && "removed file should be purged");
  return 0;
}
